...
void UGPUPointCloudRendererComponent::OnRegister()
{
    Super::OnRegister();

    // Make sure the component is visible
    SetVisibility(true, /*propagateToChildren=*/true);
    SetHiddenInGame(false, /*propagateToChildren=*/true);

    // This is critical: UE culls procedural content if the bounds are tiny
    SetComponentTickEnabled(true);
    UpdateBounds();
    MarkRenderStateDirty();

    // Optional: ensure the component is not culled because of parent bounds
...
void UGPUPointCloudRendererComponent::AddSnapshot(TArray<FLinearColor> &pointPositions, TArray<uint8> &pointColors, FVector offsetTranslation, FRotator offsetRotation) {
	...
	// I'd like to see the transformation matrix
	UE_LOG(GPUPointCloudRenderer, Warning, TEXT("Transform objMatrix (editor script) = %s"), *objMatrix.ToString());
...
void UGPUPointCloudRendererComponent::CreateStreamingBaseMesh(int32 pointCount)
{
	...
	mBaseMesh->CastShadow = false;
	mBaseMesh->bCastDynamicShadow = false;
	mBaseMesh->bCastStaticShadow = false;
